...
var Controller = React.createClass({displayName: "Controller",
	...
	start: function(){
		if(this.state.status==''){
			this.setState({status:'playing',text:'开 始'}, function(){
				playing = true;
				this.props.start();
			});
		}
	...
	render: function(){
		return (
			React.createElement("div", {className: "controller"}, 
				React.createElement("div", {className: "start "+this.state.status, onClick: this.start}, React.createElement("span", null, this.state.text))
			)
		);
	}
});
